...
| Password | string | NN | Plain text for now |
| FailedLoginAttempts | int | NN | Default 0 |
| LockoutUntil | datetime | nullable | Set on lockout |
| LastLoginDate | datetime | nullable | Updated on each login |
...
| EmployeeID | string | FK → crew_members.EmployeeID, NN | Composite PK |
...
| Column | Type | Constraints | Notes |
|---|---|---|---|
| AdminID | string | PK, UQ, NN | e.g. ADM001 |
| FullName | string | NN | |
| Email | string | UQ, NN | Used for login |
| Password | string | NN | Plain text for now |
| FailedLoginAttempts | int | NN | Default 0 |
...
## 12. loyalty_log.csv
> Tracks every points transaction per passenger

| Column | Type | Constraints | Notes |
|---|---|---|---|
| LogID | string | PK, UQ, NN | Auto-generated |
| PassengerID | string | FK → passengers.PassengerID, NN | |
| TicketID | string | FK → tickets.TicketID, nullable | Null if manual adjustment |
| PointsChanged | int | NN | Positive = earned, Negative = redeemed |
| Reason | string | NN | e.g. "Booking TK00012" or "Manual adjustment" |
...
| UserID | string | NN | PassengerID or AdminID |
| UserRole | string | NN | Admin / Passenger |
| ActionType | string | NN | Login / Logout / Create / Update / Delete / Book / Cancel |
| EntityAffected | string | nullable | e.g. "Flight WY101" |
| Details | string | nullable | Extra context |

---

## 14. error_log.csv
> Append-only error log — no relational links

| Column | Type | Constraints | Notes |
|---|---|---|---|
| LogID | string | PK, UQ, NN | Auto-generated |
| Timestamp | datetime | NN | |
| ErrorMessage | string | NN | Friendly description |
| StackTrace | string | NN | Full exception detail |
...
| Flight ↔ Crew Member | M:M | Partial both sides | flight_crew.csv |
...
| Passenger → Loyalty Log | 1:M | Partial on Log side | — |